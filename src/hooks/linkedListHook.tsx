...
export function useLinkedListHook() {
  ...
  const createFirstNode = useCallback((inputValue: number) => {
    ...
    // Add first node to the node list
    // setNodes((prevArray) => [...prevArray, newNode]);
  }, []);

  const createNextNode = useCallback((inputValue: number) => {
    const newNode = {
      value: inputValue,
      next: null,
    };
  ...
  const insertNodeAt = useCallback((position: number, inputValue: number) => {
    if (linkedList.head !== null) {
  ...
  const deleteNodeAt = useCallback((position: number) => {
